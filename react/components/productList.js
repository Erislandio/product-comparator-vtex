...
import { getFirstAvailable } from './product-card/utils'
...
export const ProductList = ({ edit, item, products, loading, id }) => {
  ...
  const onSelectProduct = product => {
    ...
    if (
      mainProduct !== null &&
      product.categories[1] !== mainProduct.categories[1]
    ) {
    ...
    const newState = [
      {
        ...
        skuActive: getFirstAvailable(product),
      },
      ...setNextArray,
      ...state.filter(item => {
        return item.id !== id
                  ...
                  <div className={styles.productItem}>
                    <div
                      dangerouslySetInnerHTML={{
                        __html: parseImageTag(
                          product.items[0].images[0].imageTag,
                          false
                        ),
                      }}
                    />
